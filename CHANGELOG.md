...
- 本版启用了模块兼容门禁（bridge_compat），**模块库必须同步更新**，
  否则已启用模块会被停用
- 数据备份 = 打包数据根目录

### 主要变化

- 回复速度恢复正常：不再每次把完整模块指引塞进对话（部署机实测
  73~204s → ~20s）
- 推送播报渲染与对话解耦：模块送来的播报素材经独立渲染通道按人设整理成
  微信消息，不再占用对话上下文；渲染失败原文直发，不吞消息
- 播报模型可切换：web 后台下拉选择，无需改配置
- 定时任务与开机自启多平台可用（macOS 待测试）
- 首次使用有完整网页向导；Windows 一键包内置 opencode，Linux 提供 deb

### 修复

- 模块设置与高级配置"保存了却不生效"（issue #4/#5/#9）
- 打开管理网页偶尔卡死（版本检测改后台）
- 新添加的模块卡片首次点击无反应

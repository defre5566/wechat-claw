...
function openModal(name) {
  if (name === "logs") {
    ...
    $("[data-source-add]", node).onclick = async () => { try { await api.post("/api/admin/sources/add", { type: $("#sourceType", node).value, url: $("#sourceUrl", node).value.trim() }); node.remove(); state.sources = (await api.get("/api/admin/sources")).sources || []; render(); toast("模块源已添加", "success"); } catch (e) { toast(e.message, "error"); } };
